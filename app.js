...
function Navbar() {
  ...
  const navLinks = [
    ...
    { label: "CREATIVE LAB", short: "05", icon: "◎", href: "#creative-lab", id: "creative-lab" },
...
// ==========================================
// 12. DJI OSMO MOBILE 7 3D GIMBAL (THREE.JS WEBGL SHOWCASE)
// ==========================================
function DjiGimbalCanvas({ hoveredCard }) {
  const containerRef = useRef(null);
  const canvasRef = useRef(null);
  const isDraggingRef = useRef(false);
  const previousMousePositionRef = useRef({ x: 0, y: 0 });
  const manualRotOffsetRef = useRef({ x: 0, y: 0 });
  ...
  useEffect(() => {
    ...
    // Lighting Setup (Studio Showcase)
    ...
    const fillLight = new THREE.DirectionalLight(0xebf2fa, 0.65);
    ...
    const bottomBounce = new THREE.DirectionalLight(0xfef2e8, 0.3);
    bottomBounce.position.set(0, -4, 2);
    scene.add(bottomBounce);

    // Master Gimbal Group
    const gimbalMasterGroup = new THREE.Group();
    scene.add(gimbalMasterGroup);
    ...
    const whitePearlMat = new THREE.MeshPhysicalMaterial({
      color: 0xffffff,
      roughness: 0.18,
      metalness: 0.08,
      clearcoat: 0.4,
    ...
    const ledCyanMat = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
    const ledRedMat = new THREE.MeshBasicMaterial({ color: 0xff2b44 });
    const orangeAccentMat = new THREE.MeshStandardMaterial({ color: 0xff5500, roughness: 0.35, metalness: 0.1 });

    const phoneObsidianMat = new THREE.MeshPhysicalMaterial({
      color: 0x111317,
      roughness: 0.12,
      metalness: 0.85,
      clearcoat: 0.8
    ...
    handleGroup.position.set(0, -0.9, 0);

    // Main ergonomic tapered cylinder grip
    const gripGeom = new THREE.CylinderGeometry(0.36, 0.42, 2.3, 32);
    const gripMesh = new THREE.Mesh(gripGeom, whiteBodyMat);
    gripMesh.castShadow = true;
    gripMesh.receiveShadow = true;
    handleGroup.add(gripMesh);

    // Upper grip collar flare
    const flareGeom = new THREE.CylinderGeometry(0.45, 0.36, 0.35, 32);
    const flareMesh = new THREE.Mesh(flareGeom, whiteBodyMat);
    flareMesh.position.y = 1.25;
    handleGroup.add(flareMesh);

    // Rear rubber ergonomic contour insert
    const rearGripGeom = new THREE.CylinderGeometry(0.37, 0.425, 2.0, 24, 1, false, 0, Math.PI);
    const rearGripMesh = new THREE.Mesh(rearGripGeom, rubberGripMat);
    rearGripMesh.rotation.y = -Math.PI / 2;
    rearGripMesh.position.z = -0.01;
    handleGroup.add(rearGripMesh);

    // Bottom 1/4" tripod collar ring in titanium
    const tripodRingGeom = new THREE.CylinderGeometry(0.4, 0.4, 0.22, 32);
    const tripodRingMesh = new THREE.Mesh(tripodRingGeom, titaniumMat);
    tripodRingMesh.position.y = -1.22;
    handleGroup.add(tripodRingMesh);

    // Bottom rubberized base cap
    const baseCapGeom = new THREE.CylinderGeometry(0.38, 0.34, 0.12, 32);
    const baseCapMesh = new THREE.Mesh(baseCapGeom, rubberGripMat);
    baseCapMesh.position.y = -1.35;
    handleGroup.add(baseCapMesh);

    // Front Control Panel (Pill plate)
    const panelGeom = new THREE.BoxGeometry(0.48, 1.25, 0.08);
    const panelMesh = new THREE.Mesh(panelGeom, darkGraphiteMat);
    panelMesh.position.set(0, 0.5, 0.36);
    handleGroup.add(panelMesh);

    // OLED Telemetry Display Screen
    const oledGeom = new THREE.BoxGeometry(0.38, 0.32, 0.04);
    const oledMat = new THREE.MeshStandardMaterial({
      color: 0x080b0f,
      roughness: 0.1,
      metalness: 0.9
    });
    const oledMesh = new THREE.Mesh(oledGeom, oledMat);
    oledMesh.position.set(0, 0.88, 0.39);
    handleGroup.add(oledMesh);

    // Glowing OLED Status Elements
    const ledDotGeom = new THREE.SphereGeometry(0.028, 12, 12);
    const ledDotMesh = new THREE.Mesh(ledDotGeom, ledCyanMat);
    ledDotMesh.position.set(-0.12, 0.94, 0.42);
    handleGroup.add(ledDotMesh);

    const ledDot2Mesh = new THREE.Mesh(ledDotGeom, ledCyanMat);
    ledDot2Mesh.position.set(-0.04, 0.94, 0.42);
    handleGroup.add(ledDot2Mesh);

    const recDotMesh = new THREE.Mesh(new THREE.SphereGeometry(0.024, 12, 12), ledRedMat);
    recDotMesh.position.set(0.12, 0.94, 0.42);
    handleGroup.add(recDotMesh);

    // 5-Way Thumbstick / Joystick
    const joyBaseGeom = new THREE.SphereGeometry(0.12, 16, 16);
    const joyBaseMesh = new THREE.Mesh(joyBaseGeom, darkGraphiteMat);
    joyBaseMesh.position.set(0, 0.52, 0.4);
    handleGroup.add(joyBaseMesh);

    const joyStickGeom = new THREE.CylinderGeometry(0.04, 0.04, 0.12, 16);
    const joyStickMesh = new THREE.Mesh(joyStickGeom, titaniumMat);
    joyStickMesh.rotation.x = Math.PI / 2;
    joyStickMesh.position.set(0, 0.52, 0.48);
    handleGroup.add(joyStickMesh);

    const joyCapGeom = new THREE.CylinderGeometry(0.09, 0.09, 0.03, 20);
    const joyCapMesh = new THREE.Mesh(joyCapGeom, rubberGripMat);
    joyCapMesh.rotation.x = Math.PI / 2;
    joyCapMesh.position.set(0, 0.52, 0.54);
    handleGroup.add(joyCapMesh);

    // Shutter / Record Button (White bezel + Red center pip)
    const recBezelGeom = new THREE.CylinderGeometry(0.1, 0.1, 0.03, 24);
    const recBezelMesh = new THREE.Mesh(recBezelGeom, whitePearlMat);
    recBezelMesh.rotation.x = Math.PI / 2;
    recBezelMesh.position.set(0.13, 0.22, 0.4);
    handleGroup.add(recBezelMesh);

    const recPipGeom = new THREE.CylinderGeometry(0.055, 0.055, 0.04, 16);
    const recPipMesh = new THREE.Mesh(recPipGeom, ledRedMat);
    recPipMesh.rotation.x = Math.PI / 2;
    recPipMesh.position.set(0.13, 0.22, 0.41);
    handleGroup.add(recPipMesh);

    // M (Mode) Button
    const mBtnGeom = new THREE.BoxGeometry(0.13, 0.07, 0.03);
    const mBtnMesh = new THREE.Mesh(mBtnGeom, whitePearlMat);
    mBtnMesh.position.set(-0.13, 0.22, 0.41);
    handleGroup.add(mBtnMesh);

    // Side Knurled Focus/Zoom Dial (Left Side)
    const dialGeom = new THREE.CylinderGeometry(0.3, 0.3, 0.14, 32);
    const dialMesh = new THREE.Mesh(dialGeom, darkGraphiteMat);
    dialMesh.rotation.z = Math.PI / 2;
    dialMesh.position.set(-0.4, 0.52, 0);
    handleGroup.add(dialMesh);

    const dialRingGeom = new THREE.CylinderGeometry(0.31, 0.31, 0.04, 32);
    const dialRingMesh = new THREE.Mesh(dialRingGeom, titaniumMat);
    dialRingMesh.rotation.z = Math.PI / 2;
    dialRingMesh.position.set(-0.45, 0.52, 0);
    handleGroup.add(dialRingMesh);

    // Rear Index Finger Trigger
    const triggerGeom = new THREE.BoxGeometry(0.2, 0.38, 0.22);
    const triggerMesh = new THREE.Mesh(triggerGeom, darkGraphiteMat);
    triggerMesh.position.set(0, 0.75, -0.38);
    handleGroup.add(triggerMesh);

    gimbalMasterGroup.add(handleGroup);

    // 3. Pan Axis Motor & Articulated Fold Hinge Group
    const panGroup = new THREE.Group();
    panGroup.position.set(0, 0.48, 0);

    const panMotorGeom = new THREE.CylinderGeometry(0.42, 0.42, 0.46, 32);
    const panMotorMesh = new THREE.Mesh(panMotorGeom, whitePearlMat);
    panGroup.add(panMotorMesh);

    const panTrimGeom = new THREE.CylinderGeometry(0.43, 0.43, 0.05, 32);
    const panTrimMesh = new THREE.Mesh(panTrimGeom, titaniumMat);
    panTrimMesh.position.y = -0.15;
    panGroup.add(panTrimMesh);

    // Alignment pip
    const alignPipGeom = new THREE.BoxGeometry(0.04, 0.08, 0.02);
    const alignPipMesh = new THREE.Mesh(alignPipGeom, orangeAccentMat);
    alignPipMesh.position.set(0, 0.05, 0.43);
    panGroup.add(alignPipMesh);

    gimbalMasterGroup.add(panGroup);

    // 4. Articulated Cantilever Roll Arm
    const rollArmGroup = new THREE.Group();
    rollArmGroup.position.set(0.08, 0.8, 0.05);

    // Slanted cantilever arm structure
    const armLowerGeom = new THREE.BoxGeometry(0.34, 0.9, 0.28);
    const armLowerMesh = new THREE.Mesh(armLowerGeom, whiteBodyMat);
    armLowerMesh.rotation.z = -0.42;
    armLowerMesh.position.set(0.24, 0.25, 0);
    rollArmGroup.add(armLowerMesh);

    const armUpperGeom = new THREE.BoxGeometry(0.32, 0.85, 0.26);
    const armUpperMesh = new THREE.Mesh(armUpperGeom, whiteBodyMat);
    armUpperMesh.rotation.z = 0.35;
    armUpperMesh.position.set(0.48, 0.88, 0);
    rollArmGroup.add(armUpperMesh);

    // Roll Motor Joint at top of arm
    const rollMotorGeom = new THREE.CylinderGeometry(0.34, 0.34, 0.44, 32);
    const rollMotorMesh = new THREE.Mesh(rollMotorGeom, titaniumMat);
    rollMotorMesh.rotation.x = Math.PI / 2;
    rollMotorMesh.position.set(0.46, 1.25, 0.1);
    rollArmGroup.add(rollMotorMesh);

    const rollMotorCapGeom = new THREE.CylinderGeometry(0.35, 0.35, 0.1, 32);
    const rollMotorCapMesh = new THREE.Mesh(rollMotorCapGeom, whitePearlMat);
    rollMotorCapMesh.rotation.x = Math.PI / 2;
    rollMotorCapMesh.position.set(0.46, 1.25, 0.3);
    rollArmGroup.add(rollMotorCapMesh);

    // Subtle DJI / OM laser engraved badge plate
    const badgeGeom = new THREE.BoxGeometry(0.18, 0.35, 0.02);
    const badgeMesh = new THREE.Mesh(badgeGeom, darkGraphiteMat);
    badgeMesh.rotation.z = -0.42;
    badgeMesh.position.set(0.32, 0.32, 0.15);
    rollArmGroup.add(badgeMesh);

    gimbalMasterGroup.add(rollArmGroup);

    // 5. Tilt Axis Motor & Magnetic Clamp Mount
    const tiltGroup = new THREE.Group();
    tiltGroup.position.set(0.05, 2.05, 0.15);

    // Tilt Motor hub
    const tiltMotorGeom = new THREE.CylinderGeometry(0.34, 0.34, 0.42, 32);
    const tiltMotorMesh = new THREE.Mesh(tiltMotorGeom, whitePearlMat);
    tiltMotorMesh.rotation.z = Math.PI / 2;
    tiltGroup.add(tiltMotorMesh);

    const tiltRingGeom = new THREE.CylinderGeometry(0.35, 0.35, 0.04, 32);
    const tiltRingMesh = new THREE.Mesh(tiltRingGeom, titaniumMat);
    tiltRingMesh.rotation.z = Math.PI / 2;
    tiltRingMesh.position.x = 0.12;
    tiltGroup.add(tiltRingMesh);

    // DJI OM Magnetic Quick-Release Clamp
    const magBaseGeom = new THREE.CylinderGeometry(0.44, 0.44, 0.08, 32);
    const magBaseMesh = new THREE.Mesh(magBaseGeom, titaniumMat);
    magBaseMesh.rotation.x = Math.PI / 2;
    magBaseMesh.position.set(-0.15, 0, 0.1);
    tiltGroup.add(magBaseMesh);

    const magRingGeom = new THREE.TorusGeometry(0.36, 0.03, 16, 32);
    const magRingMesh = new THREE.Mesh(magRingGeom, orangeAccentMat);
    magRingMesh.position.set(-0.15, 0, 0.15);
    tiltGroup.add(magRingMesh);

    // Clamp spring brackets (top and bottom grips)
    const clampTopGeom = new THREE.BoxGeometry(0.18, 0.3, 0.18);
    const clampTopMesh = new THREE.Mesh(clampTopGeom, titaniumMat);
    clampTopMesh.position.set(-0.15, 1.25, 0.18);
    tiltGroup.add(clampTopMesh);

    const clampBottomGeom = new THREE.BoxGeometry(0.18, 0.3, 0.18);
    const clampBottomMesh = new THREE.Mesh(clampBottomGeom, titaniumMat);
    clampBottomMesh.position.set(-0.15, -1.25, 0.18);
    tiltGroup.add(clampBottomMesh);

    // 6. Mounted Flagship Smartphone with Pro Camera HUD Screen
    const phoneGroup = new THREE.Group();
    phoneGroup.position.set(-0.15, 0, 0.22);

    // Phone Body
    const phoneGeom = new THREE.BoxGeometry(1.65, 3.3, 0.12);
    const phoneMesh = new THREE.Mesh(phoneGeom, phoneObsidianMat);
    phoneGroup.add(phoneMesh);

    // Titanium Edge Bezel
    const bezelGeom = new THREE.BoxGeometry(1.67, 3.32, 0.06);
    const bezelMesh = new THREE.Mesh(bezelGeom, titaniumMat);
    phoneGroup.add(bezelMesh);

    // Rear Camera Module (facing back)
    const camPlateGeom = new THREE.BoxGeometry(0.68, 0.72, 0.06);
    const camPlateMesh = new THREE.Mesh(camPlateGeom, phoneObsidianMat);
    camPlateMesh.position.set(0.38, 1.15, -0.08);
    phoneGroup.add(camPlateMesh);

    // 3 Camera Lenses on rear
    const lensGeom = new THREE.CylinderGeometry(0.11, 0.11, 0.06, 24);

    const l1 = new THREE.Mesh(lensGeom, lensGlassMat);
    l1.rotation.x = Math.PI / 2;
    l1.position.set(0.28, 1.3, -0.12);
    phoneGroup.add(l1);

    const l2 = new THREE.Mesh(lensGeom, lensGlassMat);
    l2.rotation.x = Math.PI / 2;
    l2.position.set(0.28, 1.0, -0.12);
    phoneGroup.add(l2);

    const l3 = new THREE.Mesh(lensGeom, lensGlassMat);
    l3.rotation.x = Math.PI / 2;
    l3.position.set(0.48, 1.15, -0.12);
    phoneGroup.add(l3);

    // Front Dynamic Camera Viewfinder Screen Texture
    ...
    const drawCameraHUD = () => {
      // Screen background: deep obsidian gradient with live sensor preview tone
      const bgGrad = hudCtx.createLinearGradient(0, 0, 0, 1024);
      bgGrad.addColorStop(0, '#0c1017');
      bgGrad.addColorStop(0.5, '#121822');
      bgGrad.addColorStop(1, '#090d14');
      hudCtx.fillStyle = bgGrad;
      hudCtx.fillRect(0, 0, 512, 1024);

      // Rule of thirds subtle grid lines
      hudCtx.strokeStyle = 'rgba(255, 255, 255, 0.12)';
      hudCtx.lineWidth = 1.5;
      hudCtx.beginPath();
      hudCtx.moveTo(170, 0); hudCtx.lineTo(170, 1024);
      hudCtx.moveTo(342, 0); hudCtx.lineTo(342, 1024);
      hudCtx.moveTo(0, 341); hudCtx.lineTo(512, 341);
      hudCtx.moveTo(0, 682); hudCtx.lineTo(512, 682);
      hudCtx.stroke();

      // Active Target Focus Reticle [ + ] in Cyan
      hudCtx.strokeStyle = '#00f2fe';
      hudCtx.lineWidth = 2.5;
      hudCtx.strokeRect(216, 472, 80, 80);
      hudCtx.beginPath();
      hudCtx.moveTo(256, 492); hudCtx.lineTo(256, 532);
      hudCtx.moveTo(236, 512); hudCtx.lineTo(276, 512);
      hudCtx.stroke();

      hudCtx.fillStyle = '#00f2fe';
      hudCtx.font = 'bold 16px "JetBrains Mono", monospace';
      hudCtx.fillText('AF-C LOCK • 3-AXIS ON', 160, 580);

      // Top Status Bar
      hudCtx.fillStyle = '#ff2b44';
      hudCtx.beginPath();
      hudCtx.arc(42, 54, 8, 0, Math.PI * 2);
      hudCtx.fill();

      hudCtx.fillStyle = '#ffffff';
      hudCtx.font = 'bold 20px "JetBrains Mono", monospace';
      hudCtx.fillText('REC 00:01:24', 60, 60);

      hudCtx.font = 'bold 18px "JetBrains Mono", monospace';
      hudCtx.fillText('4K 60 FPS', 370, 60);

      hudCtx.fillStyle = 'rgba(255, 255, 255, 0.7)';
      hudCtx.font = '14px "JetBrains Mono", monospace';
      hudCtx.fillText('PRORES 422 HQ • CINELOG', 42, 92);
      hudCtx.fillText('BAT 96% ⚡', 390, 92);

      // Audio Meter Level Bars
      hudCtx.fillStyle = 'rgba(255, 255, 255, 0.15)';
      hudCtx.fillRect(42, 120, 180, 8);
      hudCtx.fillRect(42, 134, 180, 8);

      hudCtx.fillStyle = '#10b981';
      hudCtx.fillRect(42, 120, 140, 8);
      hudCtx.fillRect(42, 134, 130, 8);
      hudCtx.fillStyle = '#f59e0b';
      hudCtx.fillRect(182, 120, 25, 8);

      // Bottom Camera Controls Telemetry
      hudCtx.fillStyle = '#ffffff';
      hudCtx.font = 'bold 16px "JetBrains Mono", monospace';
      hudCtx.fillText('1x  0.5x  3x  5x', 190, 930);

      hudCtx.fillStyle = 'rgba(255, 255, 255, 0.85)';
      hudCtx.font = '15px "JetBrains Mono", monospace';
      hudCtx.fillText('ISO 100   1/120s   f/1.8   WB 5600K', 80, 975);
    };
    ...
    const screenGeom = new THREE.PlaneGeometry(1.58, 3.22);
    const screenMesh = new THREE.Mesh(screenGeom, screenMat);
    screenMesh.position.set(0, 0, 0.065);
    phoneGroup.add(screenMesh);

    tiltGroup.add(phoneGroup);
    gimbalMasterGroup.add(tiltGroup);

    // Initial gimbal pose
    gimbalMasterGroup.rotation.y = 0.1;
    gimbalMasterGroup.rotation.x = 0.05;
    gimbalMasterGroup.position.y = -0.15;
    ...
    const onPointerMove = (e) => {
      ...
      manualRotOffsetRef.current.y += deltaX * 0.008;
      manualRotOffsetRef.current.x += deltaY * 0.008;
    ...
    const animate = () => {
      ...
      if (hoveredCard === 'editor') {
        ...
        cardRotX = 0.08;
        cardRotZ = 0.04;
      } else if (hoveredCard === 'videography') {
        cardRotY = 0.0;
        cardRotX = -0.12;
        cardRotZ = 0.0;
      } else if (hoveredCard === 'photography') {
        cardRotY = 0.38;
        cardRotX = 0.08;
        cardRotZ = -0.04;
      }

      // Idle float & mouse parallax calculations
      if (!prefersReducedMotion) {
        const idleFloat = Math.sin(elapsedTime * 1.6) * 0.08;
        gimbalMasterGroup.position.y = -0.15 + idleFloat;

        const idleSlowTurn = hoveredCard ? 0 : Math.sin(elapsedTime * 0.4) * 0.12;

        targetRotY = cardRotY + (hoveredCard ? 0 : mouseX * 0.38 + idleSlowTurn) + manualRotOffsetRef.current.y;
        targetRotX = cardRotX + (hoveredCard ? 0 : -mouseY * 0.22) + manualRotOffsetRef.current.x;
        targetRotZ = cardRotZ + Math.sin(elapsedTime * 0.8) * 0.02;

        // Damped motorized lerp
        currentRotY += (targetRotY - currentRotY) * 0.055;
        currentRotX += (targetRotX - currentRotX) * 0.055;
        currentRotZ += (targetRotZ - currentRotZ) * 0.055;

        // Apply smooth rotation
        gimbalMasterGroup.rotation.y = currentRotY;
        gimbalMasterGroup.rotation.x = currentRotX;
        gimbalMasterGroup.rotation.z = currentRotZ;

        // Subtle pan motor and roll arm articulation
        panGroup.rotation.y = currentRotY * 0.35;
        tiltGroup.rotation.x = currentRotX * 0.4;
      }
  ...
  return (
    <div ref={containerRef} className="gimbal-3d-stage">
      <div className="gimbal-stage-glow" />
      <canvas ref={canvasRef} className="gimbal-canvas relative z-10" />

      <div className="gimbal-telemetry-badge">
        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
        <span>DJI OSMO MOBILE 7 • 3-AXIS ACTIVE STABILIZATION</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="creative-card-wrapper h-full">
      ...
      >
        ...
        {/* Software Used Section (NEW ADDITION - Subtle, Muted, Expands on Hover/Click) */}
...
// ==========================================
// 14. CREATIVE LAB SECTION (NEW ADDITION)
// ==========================================
function CreativeLab() {
  const [hoveredCard, setHoveredCard] = useState(null);
  const data = useMemo(() => window.PORTFOLIO_DATA?.creativeLab || null, []);
  const categories = useMemo(() => data?.categories || [], [data]);

  return (
    <section id="creative-lab" className="py-28 px-6 md:px-12 relative bg-white border-t border-slate-200 creative-lab-section">
      <div className="max-w-6xl mx-auto">
        {/* Section Header Badge */}
        <div className="flex items-center space-x-3 mb-6 reveal-left-on-scroll">
          <span className="text-xs font-mono font-bold text-black tracking-widest uppercase">
            {data?.badge || "04 • VISUAL & MOTION LAB"}
          </span>
          <div className="h-[1px] flex-1 bg-slate-200" />
        </div>
        ...
        {/* 3D DJI Osmo Mobile 7 Gimbal Showcase */}
        <div className="reveal-scale-on-scroll mb-12">
          <DjiGimbalCanvas hoveredCard={hoveredCard} />
        </div>
    ...
    <footer className="py-16 px-6 md:px-12 bg-slate-900 text-white border-t border-slate-800 relative">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <nav className="flex flex-wrap items-center gap-6 text-xs font-mono text-slate-300">
          ...
          <a href="#creative-lab" className="hover:text-white transition-colors">CREATIVE LAB</a>
